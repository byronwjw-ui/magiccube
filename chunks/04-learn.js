(function(){
  var STATE = { cat:'OLL', sub:'ALL', keyword:'' };
  ...
  function diffStars(d){ return '<span class="t-muted text-xs" title="难度">' + '★'.repeat(d) + '☆'.repeat(3-d) + '</span>'; }
  ...
  function cardHtml(f){
    ...
    return '<a class="card hover formula-card" href="' + href + '" data-fid="' + escapeHtml(f.id) + '" data-locked="' + (locked?'1':'0') + '">' +
      '<div class="corner">' + (f.isPremium?'<span class="pill pill-gray">🔒 Pro</span>':'<span class="pill pill-green">免费</span>') + '</div>' +
      '<div class="flex items-center gap-3"><div class="text-2xl font-bold t-blue">#' + f.number + '</div>' +
      '<div class="flex-1"><div class="font-semibold">' + escapeHtml(f.name) + '</div>' +
      '<div class="text-xs t-muted">' + escapeHtml(f.subCategory) + '</div></div></div>' +
      '<div class="mt-3 alg-text break-words" style="font-size:0.95rem">' + maskAlg(f.algorithm, f.isPremium, user.isPremium) + '</div>' +
      '<div class="mt-3 flex items-center justify-between">' + statusBadge(status) + diffStars(f.difficulty) + '</div></a>';
  }
  function applyFilter(){
    ...
    if (kw) list = list.filter(function(f){
      return String(f.number).indexOf(kw) >= 0 || f.name.toLowerCase().indexOf(kw) >= 0 || f.subCategory.toLowerCase().indexOf(kw) >= 0;
    });
    return list;
  }
  function renderSubs(){
    var box = document.getElementById('learn-subs'); if (!box) return;
    ...
    for (var j = 0; j < btns.length; j++) btns[j].addEventListener('click', function(){
      STATE.sub = this.getAttribute('data-sub'); renderSubs(); renderListInner();
    });
  }
  function renderListInner(){
    var list = document.getElementById('learn-list'); if (!list) return;
    var rows = applyFilter();
    if (rows.length === 0) { list.innerHTML = '<div class="col-span-full center t-muted py-10">没有匹配的公式</div>'; return; }
    var html = ''; for (var i = 0; i < rows.length; i++) html += cardHtml(rows[i]);
    list.innerHTML = html;
  ...
  function bindTabs(){
    var tabs = document.getElementById('learn-tabs'); if (!tabs || tabs.__bound) return;
    tabs.__bound = true;
    var btns = tabs.querySelectorAll('button');
    for (var i = 0; i < btns.length; i++) btns[i].addEventListener('click', function(){
      STATE.cat = this.getAttribute('data-cat'); STATE.sub = 'ALL';
      var bs = tabs.querySelectorAll('button');
      for (var k = 0; k < bs.length; k++) bs[k].classList.toggle('active', bs[k] === this);
      renderSubs(); renderListInner();
    });
  }
  function bindSearch(){
    var inp = document.getElementById('learn-search'); if (!inp || inp.__bound) return;
    inp.__bound = true;
    inp.addEventListener('input', function(){ STATE.keyword = this.value; renderListInner(); });
  }
  window.renderLearn = function(){ bindTabs(); bindSearch(); renderSubs(); renderListInner(); };
})();
