(function(){
  ...
  function buildDeck(mode){
    ...
    if (mode === 'weak') {
      pool = pool.slice().sort(function(a,b){
        ...
        return (pa?pa.successRate:0) - (pb?pb.successRate:0);
      }).slice(0, 15);
    }
    pool = pool.slice();
  ...
  function renderPicker(){
    ...
    root.innerHTML = '<h1>练习</h1>' +
      ...
      '<div class="text-2xl">🎲</div><h3 class="mt-2">随机闪卡</h3>' +
      '<p class="t-muted mt-1">从已解锁的公式中随机抽 10 张。</p></button>' +
      '<button class="card hover" data-mode="oll" style="text-align:left">' +
      '<div class="text-2xl">🟡</div><h3 class="mt-2">只练 OLL</h3>' +
      '<p class="t-muted mt-1">专攻顶层翻面。</p></button>' +
      '<button class="card hover" data-mode="pll" style="text-align:left">' +
      '<div class="text-2xl">🔵</div><h3 class="mt-2">只练 PLL</h3>' +
      '<p class="t-muted mt-1">专攻顶层换位。</p></button>' +
      '<button class="card hover" data-mode="weak" style="text-align:left;position:relative">' +
      (user.isPremium ? '' : '<div class="corner" style="position:absolute;top:8px;right:8px"><span class="pill pill-gray">🔒 Pro</span></div>') +
      '<div class="text-2xl">🎯</div><h3 class="mt-2">弱项强化</h3>' +
      '<p class="t-muted mt-1">根据成功率自动推荐你最不熟的 15 张。</p></button></div>';
  ...
  function startSession(mode){
    STATE.mode = mode; STATE.deck = buildDeck(mode);
    STATE.idx = 0; STATE.success = 0; STATE.fail = 0; STATE.revealed = false; STATE.startTs = Date.now();
  ...
  function renderCard(){
    ...
    root.innerHTML = '<div class="flex items-center justify-between">' +
      '<button class="t-muted text-sm" id="btn-exit">← 退出</button>' +
      '<div class="text-sm t-muted">' + (STATE.idx + 1) + ' / ' + STATE.deck.length + '</div></div>' +
      ...
      '<div class="text-sm t-muted">答案</div><div class="alg-text mt-1 break-words">' + escapeHtml(f.algorithm) + '</div></div>' +
      (STATE.revealed
        ? '<div class="mt-6 grid grid-2"><button class="btn btn-ghost" id="btn-fail">✗ 还不熟</button>' +
          '<button class="btn btn-primary" id="btn-ok">✓ 会了</button></div>'
    ...
    mountCube(document.getElementById('practice-cube'), { alg: STATE.revealed ? f.algorithm : '', setupAlg: f.setupMoves, size: 240, tempoScale: 1 });
    var bExit = document.getElementById('btn-exit');
    bExit && bExit.addEventListener('click', function(){ if (confirm('要退出本组练习吗？已答题不会保存。')) renderPicker(); });
  ...
  function renderSummary(){
    ...
    root.innerHTML = '<div class="card center"><div class="text-5xl">🏁</div><h2 class="mt-3">本组结束！</h2>' +
      ...
      '<div class="mt-4 text-sm t-muted">正确率 ' + rate + '% · 用时 ' + durSec + ' 秒</div></div>' +
      unlockedHtml +
      '<div class="mt-4 grid grid-2"><button class="btn btn-primary" id="btn-again">再来一组</button>' +
      '<a href="#/dashboard" class="btn btn-ghost">查看仪表盘</a></div>';
